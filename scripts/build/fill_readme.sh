...
get_tag_or_commit_hash() {
    tag=$(git -C $1 describe --tags --exact-match 2>/dev/null)
    if [ $? -ne 0 ]; then
        hash=$(git -C $1 rev-parse --short HEAD)
        echo $hash
    else
        # // - replace every, / - slash, / - with, \\/ - backslash and slash
        tag=${tag////\\/}
        echo $tag
    fi
}

ESP32_ARDUINO_MATTER_BUILDER_VERSION=$(get_tag_or_commit_hash ".")
